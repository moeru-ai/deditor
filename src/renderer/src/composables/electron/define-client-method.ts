...
export function defineClientMethod<TMethods, TMethodName extends keyof TMethods>(method: TMethodName) {
  ...
  function call(...params: P): Promise<Awaited<R>> {
    return new Promise<Awaited<R>>((resolve, reject) => {
      ...
      onResponseCleanup = window.electron.ipcRenderer.on(`response:${strings.kebabcase(String(method))}`, (_, res: Awaited<R>) => {
        resolve(res)
        onResponseCleanup?.()
      })
      onErrorCleanup = window.electron.ipcRenderer.on(`response:error:${strings.kebabcase(String(method))}`, (_, err: Error) => {
        reject(err)
        onErrorCleanup?.()
      })
    })
