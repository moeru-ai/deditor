...
export default defineConfig({
  ...
  hooks: {
    'build:done': async () => {
      const iconSetData = await importDirectory(join(cwd(), 'src', 'icons'), { prefix: 'deditor-icons', ignoreImportErrors: 'warn' })
      ...
      await writeFile(join('dist', 'info.json'), json({
        prefix: 'deditor-icons',
        name: 'Deditor.app supported Icons',
